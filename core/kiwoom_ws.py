...
class KiwoomWebSocket:
    ...
    async def run(self) -> None:
        ...
        while self._running:
            try:
                async with websockets.connect(WS_URL, ssl=True, open_timeout=10) as ws:
                    self._ws = ws
                    await self._login(ws)
                    await self._register(ws, self._tickers)
                    await self._recv_loop(ws)
            except Exception as e:
                logger.warning("WebSocket 연결 끊김: {} — 5초 후 재연결", e)
...
def main():
    ...
    parser.add_argument("--tickers", nargs="+",
                        default=["005930","000660","035420","051910","005380"])
    args = parser.parse_args()

    logger.info("실시간 시세 수신 시작 | 종목: {}", args.tickers)
    logger.info("Ctrl+C 로 종료")

    client = KiwoomWebSocket(tickers=args.tickers)
    try:
